...
```
procedure hotPointChecker();
	every 10 seconds;
	do if i am master;
		maxVisitedRegion = get max visited region in 10 seconds;
        minVisitedRegion = get min visited region in 10 seconds;
        if (writableRegionCount >= 2 && maxCount >= 2 * minCount && maxCount > HOTPOINT_THRESHOLD);
        	for every table in maxVisitedRegion;
